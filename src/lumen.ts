import { AttachmentPayload, Client, ClientEvents, IntentsBitField, Message } from 'discord.js';
...
export class Lumen {
    ...
    public defaultConfig(): GuildConfig {
        return parse(fs.readFileSync(`${this.config_dir}/default.toml`, 'utf-8')) as never;
    }

    public getConfig(guild_id: string): GuildConfig {
        try {
            return deepAssign({}, this.defaultConfig(), parse(fs.readFileSync(`${this.config_dir}/${guild_id}.toml`, 'utf-8')) as never) as never;
        } catch {
    ...
    public getGuildData(guild_id: string): GuildData {
        try {
            return parse(fs.readFileSync(`${this.data_dir}/${guild_id}.toml`, 'utf-8')) as never;
        } catch {
    ...
    public setGuildData(guild_id: string, data: GuildData): void {
        fs.writeFileSync(`${this.data_dir}/${guild_id}.toml`, stringify(data as never));
    }
    ...
    public parseMessagePrefix(message: Message<true>): [string, string] | null {
        const found = this.getPrefixes(message.guildId).find((x) => message.content.startsWith(x));
        if (found !== undefined) {
    ...
    public async onMessageCreate(message: Message) {
        ...
        if (
            ...
            if (message.content === `${this.config.config_prefix}config upload` && message.attachments.size > 0) {
                const req = await fetch(message.attachments.first()!.url);
                const data = await req.text();
                const uploaded = parse(data) as never as GuildConfig;
                await fs.promises.writeFile(`${this.config_dir}/${message.guild.id}.toml`, data);

                return void await message.reply(`:white_check_mark: Uploaded! This server\'s prefixes are ${uploaded.prefixes.map(x => `\`${x}\``).join(', ')}`);
            }

            if (message.content === `${this.config.config_prefix}config reset`) {
                await fs.promises.rm(`${this.config_dir}/${message.guild.id}.toml`);
                return void await message.reply(':white_check_mark: This server\'s config has been reset.');
            }

            if (message.content === `${this.config.config_prefix}config get`) {
                try {
                    const data = await fs.promises.readFile(`${this.config_dir}/${message.guild.id}.toml`, 'utf-8');
                    return void await message.reply({ files: [{ attachment: Buffer.from(data, 'utf-8'), name: 'config.toml' } as AttachmentPayload] });
                } catch {
                    const data = await fs.promises.readFile(`${this.config_dir}/default.toml`, 'utf-8');
                    return void await message.reply({
                        content: ':warning: This server has no config. The default one has been provided.',
                        files: [{ attachment: Buffer.from(data, 'utf-8'), name: 'config_default.toml' } as AttachmentPayload]
                    });
                }
            }
        }
        ...
        for (const [name, command] of this.commandMap()) {
            const ctxt = new Ctxt(this, message, this.getConfig(message.guildId));
            if (vecs.startsWith(name)) {
                ...
                try {
                    ...
                    if (!check) {
                        return await message.reply(':warning: `You are not allowed to use this command.`');
                    }
                } catch (e) {
                    return await message.reply(`:x: Preprocessing error: \`${(e as Error).message || e as string}\``);
                }

                try {
                    const argv = [] as never[];
                    const pctx = new ParseCtxt(new SplitAsciiWhitespace(argl), ctxt);
                    for (const parser of command.parsers) {
                        try {
                            argv.push(await parser.parse(pctx) as never);
                        } catch (e) {
                            return await message.reply(`:x: Parsing error: \`${(e as Error).message || e as string}\``);
                        }
                    }
                    await command.execute(ctxt, ...argv);
                } catch (e) {
                    return await message.reply(`:warning: \`${(e as Error).message || e as string}\``);
                }
                break;
            }
        }
    ...
    public event<Event extends keyof ClientEvents>(event: Event, listener: (...args: ClientEvents[Event]) => unknown) {
